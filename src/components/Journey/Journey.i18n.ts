/* src/components/Journey/Journey.i18n.ts */

export const translations: Record<string, any> = {
    vi: {
        preparing: 'Chờ phòng',
        inProgress: 'Đang làm',
        completed: 'Kiểm đồ',
        feedback: 'Đánh giá',
        // Stepper 4 bước
        footSoak: 'Ngâm chân',
        service: 'Dịch vụ',
        checkItems: 'Kiểm đồ',
        rating: 'Đánh giá',
        todayServices: 'Dịch vụ hôm nay',
        orderId: 'Mã đơn hàng',
        welcomeMessage: 'Mời bạn ngâm chân thảo dược, chườm túi nóng, uống tách trà và cho chúng tôi ít phút để chuẩn bị phòng.',
        yourJourney: 'Lộ trình dịch vụ',
        step1Title: 'Ngâm chân thảo dược',
        step1Sub: 'Rửa chân, tẩy da chết bằng bột thảo dược',
        step2Title: 'Thực hiện dịch vụ',
        step2Sub: 'Nhân viên phục vụ tại phòng',
        step3Title: 'Kiểm tra tư trang',
        step3Sub: 'Kiểm tra đồ trước khi ra về',
        step4Title: 'Đánh giá dịch vụ',
        step4Sub: 'Chia sẻ trải nghiệm của bạn',
        room: 'Phòng',
        bed: 'Giường',
        staff: 'NV',
        minutes: 'phút',
        title: 'Hành trình thư giãn',
        errorTitle: 'Đã xảy ra lỗi',
        retry: 'Thử lại',
        finishTitle: 'Hoàn tất!',
        finishSub: 'Cảm ơn bạn đã trải nghiệm dịch vụ.',
        goHome: 'Về trang chủ',
        redirecting: 'Đang chuyển hướng...',
        autoRedirect: 'Tự động chuyển sau vài giây',
        spa_service_fallback: 'Dịch vụ Spa',
        sos: 'KHẨN CẤP',
        sosConfirm: 'Bạn cần hỗ trợ gấp tại phòng?',
        sosSending: 'Đang báo quầy...',
        sosSent: 'Đã báo lễ tân!',
        check_phone: 'Điện thoại',
        check_wallet: 'Ví tiền',
        check_belongings: 'Tư trang',
        check_message: 'Xin quý khách trước khi rời phòng vui lòng kiểm tra kỹ tư trang (điện thoại, ví, trang sức...) để đảm bảo không bỏ quên món đồ nào nhé!',
        check_confirm: 'Tôi đã kiểm tra xong',
        // ActiveService / ServiceList
        activeService: 'Dịch vụ đang thực hiện',
        elapsed: 'Thời gian đã trôi qua',
        remaining: 'Còn lại',
        feedbackTitle: 'Góp ý nhanh để KTV sửa lỗi:',
        optional: '(Tùy chọn)',
        therapistLabel: 'Nhân viên',
        addService: 'Thêm dịch vụ',
        changeTherapist: 'Đổi nhân viên',
        sosBtn: 'BÁO KHẨN CẤP',
        sosSentBtn: 'ĐÃ BÁO LỄ TÂN',
        notified: 'ĐÃ BÁO QUẦY',
        services: 'Dịch vụ',
        waiting: 'Chờ bắt đầu',
        done: 'Hoàn thành',
        serviceGroups: 'nhóm dịch vụ',
        quickFeedback: 'Góp ý nhanh',
        sent: 'Đã báo',
        addServiceShort: 'Thêm DV',
        changeStaffShort: 'Đổi NV',
        // Rating view
        rateTitle: 'Đánh giá dịch vụ',
        rateSub: 'Đánh giá từng dịch vụ để chúng tôi cải thiện tốt hơn',
        violationsSectionTitle: 'Góp ý dịch vụ (nếu có)',
        yourExperience: 'Trải nghiệm của bạn?',
        submitting: 'Đang gửi...',
        submitRating: 'Gửi đánh giá',
        selectToSubmit: 'Chọn mức độ để gửi',
        rated: 'đã đánh giá',
        ratedSent: 'Đã gửi',
        skipRating: 'Bỏ qua đánh giá →',
        processing: 'Đang xử lý...',
        thankYou: '🎉 Cảm ơn bạn đã đánh giá!',
        allDoneRedirecting: 'Đang chuyển hướng...',
        // Check belongings (inline version in ServiceList)
        beforeYouLeave: 'Nhắc nhở trước khi ra về',
        checkBeforeLeave: 'Vui lòng kiểm tra kỹ tư trang cá nhân trước khi rời phòng!',
        checkedRateNow: '✅ Đã kiểm tra — Tiến hành đánh giá',
        checkPhone: '📱 Điện thoại',
        checkWallet: '👛 Ví tiền',
        checkWatch: '⌚ Đồng hồ / Trang sức',
        checkKeys: '🔑 Chìa khóa / Thẻ',
        // TipModal
        tipExcellent: 'Xuất sắc!',
        tipMessage: 'Cảm ơn vì sự hài lòng của bạn, bạn có thể gửi tiền tip cho nhân viên xem như phần thưởng khích lệ.',
        tipSend: 'Gửi tiền tip',
        tipSkip: 'Bỏ qua',
        tipProcessing: 'Đang xử lý...',
    },
    en: {
        preparing: 'Preparing',
        inProgress: 'In Service',
        completed: 'Check Belongings',
        feedback: 'Feedback',
        footSoak: 'Foot Soak',
        service: 'Service',
        checkItems: 'Check Items',
        rating: 'Rating',
        todayServices: "Today's Services",
        orderId: 'Order ID',
        welcomeMessage: 'Please enjoy herbal foot bath, hot compress, sip some tea, and give us a few minutes to prepare your room.',
        yourJourney: 'Your Journey',
        step1Title: 'Herbal Foot Soak',
        step1Sub: 'Foot wash & exfoliation with herbal powder',
        step2Title: 'Service in Progress',
        step2Sub: 'Therapist will serve you in the room',
        step3Title: 'Check Belongings',
        step3Sub: 'Check your items before leaving',
        step4Title: 'Rate Your Experience',
        step4Sub: 'Share your feedback with us',
        room: 'Room',
        bed: 'Bed',
        staff: 'Staff',
        minutes: 'min',
        title: 'Relaxation Journey',
        errorTitle: 'An Error Occurred',
        retry: 'Retry',
        finishTitle: 'Completed!',
        finishSub: 'Thank you for visiting us.',
        goHome: 'Go Home',
        redirecting: 'Redirecting...',
        autoRedirect: 'Redirecting automatically in a few seconds',
        spa_service_fallback: 'Spa Service',
        sos: 'EMERGENCY',
        sosConfirm: 'Do you need urgent assistance in the room?',
        sosSending: 'Notifying...',
        sosSent: 'Reception Notified!',
        check_phone: 'Phone',
        check_wallet: 'Wallet',
        check_belongings: 'Personal Belongings',
        check_message: 'Please double-check your personal belongings (phone, wallet, jewelry...) before leaving the room to ensure nothing is left behind.',
        check_confirm: 'I have checked everything',
        // ActiveService / ServiceList
        activeService: 'Service in Progress',
        elapsed: 'Time Elapsed',
        remaining: 'Remaining',
        feedbackTitle: 'Quick feedback to correct Therapist:',
        optional: '(Optional)',
        therapistLabel: 'Therapist',
        addService: 'Add Service',
        changeTherapist: 'Change Therapist',
        sosBtn: 'EMERGENCY SOS',
        sosSentBtn: 'RECEPTION NOTIFIED',
        notified: 'NOTIFIED',
        services: 'Services',
        waiting: 'Waiting',
        done: 'Done',
        serviceGroups: 'service groups',
        quickFeedback: 'Quick feedback',
        sent: 'Sent',
        addServiceShort: 'Add Service',
        changeStaffShort: 'Change',
        // Rating view
        rateTitle: 'Rate Your Services',
        rateSub: 'Rate each service to help us improve',
        violationsSectionTitle: 'Service feedback (if any)',
        yourExperience: 'Your experience?',
        submitting: 'Submitting...',
        submitRating: 'Submit',
        selectToSubmit: 'Select to submit',
        rated: 'rated',
        ratedSent: 'Sent',
        skipRating: 'Skip rating →',
        processing: 'Processing...',
        thankYou: '🎉 Thank you for your feedback!',
        allDoneRedirecting: 'Redirecting...',
        // Check belongings (inline version in ServiceList)
        beforeYouLeave: 'Before You Leave',
        checkBeforeLeave: 'Please check your personal belongings before leaving!',
        checkedRateNow: '✅ Checked — Rate Now',
        checkPhone: '📱 Phone',
        checkWallet: '👛 Wallet',
        checkWatch: '⌚ Watch / Jewelry',
        checkKeys: '🔑 Keys / Cards',
        // TipModal
        tipExcellent: 'Excellent!',
        tipMessage: 'Thank you for your satisfaction! You can send a tip to the therapist as an encouragement.',
        tipSend: 'Send Tip',
        tipSkip: 'Skip',
        tipProcessing: 'Processing...',
    },

    kr: {
        preparing: '준비 중',
        inProgress: '서비스 중',
        completed: '소지품 확인',
        feedback: '피드백',
        footSoak: '족욕',
        service: '서비스',
        checkItems: '소지품 확인',
        rating: '평가',
        todayServices: '오늘의 서비스',
        orderId: '주문 번호',
        welcomeMessage: '허브 족욕과 온찜질을 즐기며 따뜻한 차를 드시는 동안 객실을 준비하겠습니다. 잠시만 기다려 주세요.',
        yourJourney: '당신의 여정',
        step1Title: '허브 족욕',
        step1Sub: '허브 파우더로 발 세정 및 각질 제거',
        step2Title: '서비스 진행 중',
        step2Sub: '테라피스트가 객실에서 서비스합니다',
        step3Title: '소지품 확인',
        step3Sub: '퇴실 전 소지품을 확인해 주세요',
        step4Title: '서비스 평가',
        step4Sub: '경험을 공유해 주세요',
        title: '휴식 여정',
        errorTitle: '오류가 발생했습니다',
        retry: '다시 시도',
        finishTitle: '완료!',
        finishSub: '저희와 함께 해주셔서 감사합니다.',
        goHome: '홈으로 이동',
        redirecting: '리다이렉트 중...',
        autoRedirect: '잠시 후 자동으로 이동합니다',
        spa_service_fallback: '스파 서비스',
        room: '룸',
        bed: '베드',
        staff: '담당',
        minutes: '분',
        sos: '비상',
        sosConfirm: '객실 내 긴급 지원이 필요하십니까?',
        sosSending: '알리는 중...',
        sosSent: '리셉션 알림 완료!',
        check_phone: '휴대폰',
        check_wallet: '지갑',
        check_belongings: '소지품',
        check_message: '객실을 떠나기 전에 소지품(휴대폰, 지갑, 귀중품 등)을 다시 한번 확인하여 두고 가는 물건이 없는지 확인해 주세요.',
        check_confirm: '확인을 완료했습니다',
        // ActiveService / ServiceList
        activeService: '서비스 진행 중',
        elapsed: '경과 시간',
        remaining: '남은 시간',
        feedbackTitle: '테라피스트 피드백:',
        optional: '(선택)',
        therapistLabel: '담당자',
        addService: '서비스 추가',
        changeTherapist: '담당 변경',
        sosBtn: '긴급 SOS',
        sosSentBtn: '리셉션 알림 완료',
        notified: '알림 완료',
        services: '서비스',
        waiting: '대기 중',
        done: '완료',
        serviceGroups: '서비스 그룹',
        quickFeedback: '빠른 피드백',
        sent: '전송됨',
        addServiceShort: '서비스 추가',
        changeStaffShort: '변경',
        // Rating view
        rateTitle: '서비스 평가',
        rateSub: '각 서비스를 평가하여 개선에 도움을 주세요',
        violationsSectionTitle: '서비스 피드백 (있는 경우)',
        yourExperience: '경험은 어떠셨나요?',
        submitting: '제출 중...',
        submitRating: '평가 제출',
        selectToSubmit: '평가를 선택하세요',
        rated: '평가됨',
        ratedSent: '제출됨',
        skipRating: '평가 건너뛰기 →',
        processing: '처리 중...',
        thankYou: '🎉 피드백 감사합니다!',
        allDoneRedirecting: '리다이렉트 중...',
        // Check belongings
        beforeYouLeave: '퇴실 전 확인',
        checkBeforeLeave: '퇴실 전 소지품을 확인해 주세요!',
        checkedRateNow: '✅ 확인 완료 — 평가하기',
        checkPhone: '📱 휴대폰',
        checkWallet: '👛 지갑',
        checkWatch: '⌚ 시계 / 귀금속',
        checkKeys: '🔑 열쇠 / 카드',
        // TipModal
        tipExcellent: '훌륭합니다!',
        tipMessage: '만족하셨다니 감사합니다! 테라피스트에게 팁을 보내 격려해 주세요.',
        tipSend: '팁 보내기',
        tipSkip: '건너뛰기',
        tipProcessing: '처리 중...',
    },
    jp: {
        preparing: '準備中',
        inProgress: '施術中',
        completed: '忘れ物確認',
        feedback: 'フィードバック',
        footSoak: 'フットバス',
        service: 'サービス',
        checkItems: '忘れ物確認',
        rating: '評価',
        todayServices: '本日のサービス',
        orderId: '注文番号',
        welcomeMessage: 'ハーブの足湯、温かい湿布、お茶を楽しみながら、お部屋の準備ができるまで少々お待ちください。',
        yourJourney: 'あなたの旅',
        step1Title: 'ハーブ足湯',
        step1Sub: 'ハーブパウダーでの足洗いと角質ケア',
        step2Title: 'サービス中',
        step2Sub: 'セラピストがお部屋でサービスを提供します',
        step3Title: '忘れ物確認',
        step3Sub: '退室前に持ち物をご確認ください',
        step4Title: 'サービス評価',
        step4Sub: 'ご意見をお聞かせください',
        title: 'リラクゼーションの旅',
        errorTitle: 'エラーが発生しました',
        retry: '再試行',
        finishTitle: '完了！',
        finishSub: 'ご利用ありがとうございました。',
        goHome: 'ホームへ戻る',
        redirecting: 'リダイレクト中...',
        autoRedirect: '数秒後に自動的に移動します',
        spa_service_fallback: 'スパサービス',
        room: 'ルーム',
        bed: 'ベッド',
        staff: '担当',
        minutes: '分',
        sos: '緊急',
        sosConfirm: 'お部屋での緊急なサポートが必要ですか？',
        sosSending: '通知中...',
        sosSent: '受付に通知しました！',
        check_phone: '電話',
        check_wallet: '財布',
        check_belongings: '手荷物',
        check_message: 'お忘れ物がないよう、退室前に身の回りの品（携帯電話、財布、アクセサリーなど）を今一度ご確認ください。',
        check_confirm: '確認しました',
        // ActiveService / ServiceList
        activeService: 'サービス進行中',
        elapsed: '経過時間',
        remaining: '残り',
        feedbackTitle: 'セラピストへのフィードバック:',
        optional: '(任意)',
        therapistLabel: 'セラピスト',
        addService: 'サービス追加',
        changeTherapist: '担当変更',
        sosBtn: '緊急SOS',
        sosSentBtn: '受付通知済み',
        notified: '通知済み',
        services: 'サービス',
        waiting: '待機中',
        done: '完了',
        serviceGroups: 'サービスグループ',
        quickFeedback: 'クイックフィードバック',
        sent: '送信済み',
        addServiceShort: 'サービス追加',
        changeStaffShort: '変更',
        // Rating view
        rateTitle: 'サービス評価',
        rateSub: '各サービスを評価して改善にご協力ください',
        violationsSectionTitle: 'サービスフィードバック（ある場合）',
        yourExperience: 'ご体験はいかがでしたか？',
        submitting: '送信中...',
        submitRating: '評価を送信',
        selectToSubmit: '評価を選択してください',
        rated: '評価済み',
        ratedSent: '送信済み',
        skipRating: '評価をスキップ →',
        processing: '処理中...',
        thankYou: '🎉 フィードバックありがとうございます！',
        allDoneRedirecting: 'リダイレクト中...',
        // Check belongings
        beforeYouLeave: 'ご退室前に',
        checkBeforeLeave: 'お忘れ物がないかご確認ください！',
        checkedRateNow: '✅ 確認済み — 評価へ進む',
        checkPhone: '📱 携帯電話',
        checkWallet: '👛 財布',
        checkWatch: '⌚ 時計 / アクセサリー',
        checkKeys: '🔑 鍵 / カード',
        // TipModal
        tipExcellent: '素晴らしい！',
        tipMessage: 'ご満足いただきありがとうございます！セラピストへの励ましとしてチップを送ることができます。',
        tipSend: 'チップを送る',
        tipSkip: 'スキップ',
        tipProcessing: '処理中...',
    },
    cn: {
        preparing: '准备中',
        inProgress: '服务中',
        completed: '检查物品',
        feedback: '评价',
        footSoak: '泡脚',
        service: '服务',
        checkItems: '检查物品',
        rating: '评价',
        todayServices: '今日服务',
        orderId: '订单编号',
        welcomeMessage: '请享受草药泡脚、热敷、喝杯茶，稍等片刻，我们将为您准备房间。',
        yourJourney: '您的旅程',
        step1Title: '草药泡脚',
        step1Sub: '使用草药粉洗脚并去角质',
        step2Title: '服务进行中',
        step2Sub: '技师将在房间为您服务',
        step3Title: '检查物品',
        step3Sub: '离开前请检查您的个人物品',
        step4Title: '服务评价',
        step4Sub: '请分享您的体验',
        title: '放松之旅',
        errorTitle: '发生错误',
        retry: '重试',
        finishTitle: '完成！',
        finishSub: '感谢您选择我们的服务。',
        goHome: '返回首页',
        redirecting: '正在跳转...',
        autoRedirect: '几秒后自动跳转',
        spa_service_fallback: '水疗服务',
        room: '房间',
        bed: '床位',
        staff: '技师',
        minutes: '分钟',
        sos: '紧急',
        sosConfirm: '您在房间内需要紧急协助吗？',
        sosSending: '正在通知...',
        sosSent: '已通知前台！',
        check_phone: '手机',
        check_wallet: '钱包',
        check_belongings: '个人物品',
        check_message: '在离开房间前，请仔细检查您的个人物品（手机、钱包、首饰等），确保没有遗漏任何东西。',
        check_confirm: '我已检查完毕',
        // ActiveService / ServiceList
        activeService: '服务进行中',
        elapsed: '已用时间',
        remaining: '剩余',
        feedbackTitle: '快速反馈以改进服务:',
        optional: '(可选)',
        therapistLabel: '技师',
        addService: '添加服务',
        changeTherapist: '更换技师',
        sosBtn: '紧急SOS',
        sosSentBtn: '已通知前台',
        notified: '已通知',
        services: '服务',
        waiting: '等待中',
        done: '完成',
        serviceGroups: '服务组',
        quickFeedback: '快速反馈',
        sent: '已发送',
        addServiceShort: '添加服务',
        changeStaffShort: '更换',
        // Rating view
        rateTitle: '服务评价',
        rateSub: '请评价各项服务以帮助我们改进',
        violationsSectionTitle: '服务反馈（如有）',
        yourExperience: '您的体验如何？',
        submitting: '提交中...',
        submitRating: '提交评价',
        selectToSubmit: '请选择评分',
        rated: '已评价',
        ratedSent: '已提交',
        skipRating: '跳过评价 →',
        processing: '处理中...',
        thankYou: '🎉 感谢您的评价！',
        allDoneRedirecting: '正在跳转...',
        // Check belongings
        beforeYouLeave: '离开前提醒',
        checkBeforeLeave: '离开前请检查您的个人物品！',
        checkedRateNow: '✅ 已检查 — 开始评价',
        checkPhone: '📱 手机',
        checkWallet: '👛 钱包',
        checkWatch: '⌚ 手表 / 首饰',
        checkKeys: '🔑 钥匙 / 卡片',
        // TipModal
        tipExcellent: '太棒了！',
        tipMessage: '感谢您的满意！您可以给技师发送小费作为鼓励。',
        tipSend: '发送小费',
        tipSkip: '跳过',
        tipProcessing: '处理中...',
    },
};

export type TranslationKey = keyof typeof translations.en;
